...
**Data flow (Claude Code):** Hook stdin → canonicalize tool name → extract artifacts → check allowlist → check cache → heuristics + URL check → DecisionEngine → cache result → audit log → hook stdout JSON

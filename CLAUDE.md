...
- `packages/openclaw/` — `@gendigital/sage-openclaw`: OpenClaw plugin connector (in-process hook, gate tool). `resources/` is gitignored — generated by `pnpm build` via `sync-assets.mjs`.

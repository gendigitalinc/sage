...
function formatStatusLine(denied, flagged, lastReason, lastCategory) {
  if (denied > 0 || flagged > 0) {
    ...
    return `\u{1F6E1}\uFE0F Sage: ${parts.join(", ")}${detail}`;
  }
  return "\u{1F6E1}\uFE0F Sage: \u2705";
}
...
function main() {
  ...
  if (!sessionId) {
    process.stdout.write("\u{1F6E1}\uFE0F Sage: \u2705\n");
    return;
  }
